...
class VisionSdpaCrossAttentionSimple(nn.Module):
    def __init__(self, dim: int, num_heads: int = 16) -> None:
        super().__init__()
        self.num_heads = num_heads
    ...
    def forward(
        ...
        B, L, C = query.shape
        q = self.q(query).reshape(B, L, 1, self.num_heads, -1).permute(2, 0, 3, 1, 4).unbind(0)[0]
        k, v = self.kv(hidden_states).reshape(B, hidden_states.shape[1], 2, self.num_heads, -1).permute(2, 0, 3, 1, 4).unbind(0)
...
class Qwen2VLVisionBlockSimple(nn.Module):
    def __init__(self, embed_dim=1280, num_heads=16, mlp_ratio=4, hidden_act="quick_gelu", attn_implementation="sdpa"):
        super().__init__()
        self.norm0 = LayerNorm(embed_dim, eps=1e-6)
        ...
        self.attn = VisionSdpaCrossAttentionSimple(embed_dim, num_heads)
        self.mlp = VisionMlp(dim=embed_dim, hidden_dim=mlp_hidden_dim, hidden_act=hidden_act)

    def forward(self, query,hidden_states, cu_seqlens, rotary_pos_emb=None, position_embeddings=None):
...
class Qwen2VLVisionConnectorSimple(nn.Module):
    def __init__(self, depth, seq_len, embed_dim=1280, num_heads=16, mlp_ratio=4, hidden_act="quick_gelu", attn_implementation="sdpa"):
        ...
        # self.pos_embed2 = nn.Parameter(torch.zeros(1, 64, embed_dim))
        self.seq_len = seq_len
    def forward(self, query,hidden_states, cu_seqlens=None, rotary_pos_emb=None):
        B, _, C = hidden_states.shape
        L = self.seq_len
        # Apply positional embedding

        query = query + self.pos_embed[:,:query.shape[1]]
        # query = query + self.pos_embed2[:,:query.shape[1]]
        ...
        for block in self.blocks:
            query = block(query,hidden_states, cu_seqlens, rotary_pos_emb)
